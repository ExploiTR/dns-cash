...
bool DNSServer::start(unsigned short port) {
	try {
		...
		sock_server_addr.sin_port = htons(this->dns_port); //converts the value from the host's to network short byte order (big endian)

		inet_pton(AF_INET, "0.0.0.0", &sock_server_addr.sin_addr); //Converts IP string into a binary format for sockaddr_in.
...
void DNSServer::send_to_server(const char* resp, int resp_len) {
	...
	int ret = sendto(this->socket_, resp, resp_len, 0, (SOCKADDR*)&this->sock_remote_addr, sizeof(this->sock_remote_addr));
...
void DNSServer::send_to_client(const sockaddr_in& client_addr, const char* resp, int resp_len) {
